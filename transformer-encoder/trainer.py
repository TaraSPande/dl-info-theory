...
from collator import ClassificationCollator, RegressionCollator
...
class Trainer:
    ...
    def _build_dataloaders(self) -> Tuple[DataLoader, DataLoader]:
        ...
        if cfg.task_type == TaskType.CLASSIFICATION:
            ...
            collate = RegressionCollator(tok, max_length=cfg.max_length, fields=cfg.text_fields)
        else:
            raise ValueError(f"Unknown/unsupported task type for this trainer: {cfg.task_type}")
    ...
    def _step(self, batch: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        if self.data_config.task_type == TaskType.CLASSIFICATION:
            ...
            raise ValueError(f"Unknown/unsupported task type for this trainer: {self.data_config.task_type}")
        return loss
...
if __name__ == "__main__":
    ...
    choices = ["vanilla", "synth_dense", "synth_random", "hybrid_dense", "hybrid_random", "jto"]
    ...
    # Build a tiny demo model. Replace with *your* custom Transformer implementation.
    # vocab_size = len(tok)
    # if data_cfg.task_type == TaskType.SEQ2SEQ:
    #     model = TinySeq2Seq(vocab_size=vocab_size, d_model=args.model_dim, nhead=args.heads, num_layers=args.layers, pad_token_id=tok.pad_token_id)
    # elif data_cfg.task_type == TaskType.CAUSAL_LM:
    #     model = TinyDecoderOnlyLM(vocab_size=vocab_size, d_model=args.model_dim, nhead=args.heads, num_layers=args.layers, pad_token_id=tok.pad_token_id)
    # else:  # CLASSIFICATION
    #     backbone = TinyDecoderOnlyLM(vocab_size=vocab_size, d_model=args.model_dim, nhead=args.heads, num_layers=args.layers, pad_token_id=tok.pad_token_id)
    #     # Infer number of labels from dataset info
    ...
    #     model = TinyClassifier(backbone, num_labels=num_labels)
...
# ------------------------------------------------------------
    ...
    elif data_cfg.task_type == TaskType.REGRESSION:
        # number of regression targets equals configured label_fields length
        num_targets = len(data_cfg.label_fields or [])
